...
- **кадры** — объект кладётся у кадров, которым выпала запись
  ...
  следующий кадр этой стороны ждёт, пока обменник примет объект.

Исход у журнала — `allow`: `when=deny` на фазе без инспекторов не выпадает
никогда (кроме объектов запроса, если отказала поздняя фаза с инспекторами).
