...
`frame`: снимка ещё нет, архивировать нечего — `nginx -t`.

`reload` шире capture — только на фазе запроса. На остальных фазах
оригинал брать неоткуда: то, что за снимком, уже ушло клиенту, а
перечитать ответ второй раз нельзя — `nginx -t`.
...
-t`. Снимок остаётся обязательным (он поднимает буфер и даёт инспекторам
их вид): `waf_archive frame` без `waf_capture frame body` — `nginx -t`.
...
Инспекторы нужны той же стороне: `waf_archive frame` при `waf_inspect
frame:c2s` — `nginx -t`, нагрузку `s2c` держать и судить некому.

# правила

- нет фазы — `nginx -t`. `frame` — `nginx -t`
- `reload` шире capture вне фазы запроса — `nginx -t`
- инспекторы нужны **той же фазе**: `waf_archive response` без
  `waf_inspect response` — `nginx -t`
