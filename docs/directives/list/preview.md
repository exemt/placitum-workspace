...
Без `reload` объект не из capture или размер больше — `nginx -t`, у фазы с
инспекторами. `reload` кладёт в Redis оригинал. Инспекторы его не видят.

Инспекторы превью не нужны. Фаза, где никого не спрашивают, берёт срез из
самого трафика — со списками снимка, но без сверки со снимком, которого у
неё нет: тело запроса дочитывается ради одного превью, тело ответа
копируется префиксом по мере отдачи, у кадров — при `waf_audit_frames all`
([archive.md](archive.md#без-инспекторов)).
