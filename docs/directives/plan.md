...
Следующий круг — фаза ответа и кадры: [plan-phases.md](plan-phases.md).
...
```
waf_inspect request|response|frame <name> wave=<n> [timeout=] [mode=];
...
## 1.7 контур без смены синтаксиса
...
| `waf_on_absent pass` | `waf_exception request absent pass` |
